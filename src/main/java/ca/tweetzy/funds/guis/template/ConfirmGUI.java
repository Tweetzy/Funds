...
public final class ConfirmGUI extends FundsBaseGUI {
	...
	@Override
	protected void draw() {
		Arrays.asList(10, 11, 12).forEach(i -> setButton(i, QuickItem.of(CompMaterial.RED_STAINED_GLASS_PANE).name(TranslationManager.string(Translations.GUI_CONFIRM_ITEMS_CANCEL_NAME)).lore(TranslationManager.list(Translations.GUI_CONFIRM_ITEMS_CANCEL_LORE)).make(), click -> {
			confirmed.accept(false);
		}));

		Arrays.asList(14, 15, 16).forEach(i -> setButton(i, QuickItem.of(CompMaterial.LIME_STAINED_GLASS_PANE).name(TranslationManager.string(Translations.GUI_CONFIRM_ITEMS_CONFIRM_NAME)).lore(TranslationManager.list(Translations.GUI_CONFIRM_ITEMS_CONFIRM_LORE)).make(), click -> {
